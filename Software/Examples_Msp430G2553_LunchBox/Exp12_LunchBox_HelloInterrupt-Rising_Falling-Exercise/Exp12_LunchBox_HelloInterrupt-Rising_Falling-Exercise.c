...
void main(void)
{
    ...
    P1DIR |= (LED1 + LED2);                     // Set LED1 & LED2 pin -> Output
    ...
    P1IES &= ~SW1;              // Select SW1 switch -> Interrupt on Rising Edge
    P1IES |= SW2;              // Select SW2 switch -> Interrupt on Falling Edge
    P1IE |= (SW1 + SW2);                       // Enable Interrupt on SW pin
